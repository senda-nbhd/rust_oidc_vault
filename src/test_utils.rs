...
use crate::{
    idp::admin::IdpAdmin,
    oidc::keycloak::KeyCloakToken,
    vault::{ApiToken, VaultService},
    AiclIdentity,
};
...
pub trait AuthenticateTestRequest {
    /// Authenticate a user and return session information
    fn add_api_token(self, session: &AuthSession) -> Self;
}

impl AuthenticateTestRequest for TestRequest {
    fn add_api_token(self, session: &AuthSession) -> Self {
        if let Some(api_token) = &session.api_token {
            self.add_header("Authorization", format!("Bearer {}", api_token.client_token))
        } else {
            self
        }
    }
...
impl AuthTestUtils {
    ...
    async fn get_oidc_client(&self) -> Result<KeycloakOidcClient, anyhow::Error> {
        ...
        let issuer_url = IssuerUrl::new(format!("{}/realms/{}", self.keycloak_url, self.realm))?;

        // Discover the provider metadata
        let provider_metadata =
            KeycloakMetadata::discover_async(issuer_url.clone(), &async_http_client).await?;
    ...
    pub async fn authenticate_user(&self, user: &TestUser) -> Result<AuthSession, anyhow::Error> {
        ...
        let token_response = client
            ...
            .await
            .with_context(|| "Failed to request token")?;
        ...
        let user_id = decode_token_claims(&keycloak_token.id_token.to_string())
            .with_context(|| "unable to decode the sub claim")?;
        // Step 4: Fetch the user's identity from KeyCloak
        let identity = self
            .idp
            .get_domain_user(user_id)
            .await
            .with_context(|| "Failed to fetch user identity from KeyCloak")?;
        ...
        let api_token = match identity.role {
            crate::Role::Admin => Some(
                self.vault
                    .create_api_token_with_oidc(&identity, &keycloak_token)
                    .await?,
            ),
            crate::Role::Captain => Some(
                self.vault
                    .create_api_token_with_oidc(&identity, &keycloak_token)
                    .await?,
            ),
    ...
    pub async fn authenticate_users(
        &self,
        users: &[TestUser],
    ) -> Vec<Result<AuthSession, anyhow::Error>> {
        let futures = users
            .iter()
    ...
    pub async fn get_api_token_direct(
        ...
        let session = self
            .create_session_with_api_token(username, password)
            .await?;

        // Return the API token
        session
            .api_token
            .ok_or_else(|| anyhow::anyhow!("No API token found in session"))
    }

    pub async fn revoke_token(&self, token: &str) -> Result<(), anyhow::Error> {
        let client_settings =  VaultClientSettingsBuilder::default()
        .token(token).build()?;
        let client = VaultClient::new(client_settings)?;
        vaultrs::token::revoke_self(&client).await?;
        Ok(())
    }
}
